...
// ─── Componente para configurar el logo ──────────────────────────────────────
function LogoConfig() {
  const [url, setUrl] = useState(localStorage.getItem('uat_logo') || '')
  const [preview, setPreview] = useState(localStorage.getItem('uat_logo') || '')
  const [guardado, setGuardado] = useState(false)

  const guardar = () => {
    localStorage.setItem('uat_logo', url)
    setPreview(url)
    setGuardado(true)
  ...
  const quitar = () => {
    localStorage.removeItem('uat_logo')
    setUrl('')
    setPreview('')
    window.dispatchEvent(new Event('storage'))
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6">
      <h3 className="font-bold text-uat-blue mb-4">Logo del Sistema</h3>
      <div className="flex flex-col sm:flex-row gap-4 items-start">
        {/* Preview */}
        <div className="w-32 h-20 bg-uat-blue rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
          {preview ? (
            <img src={preview} alt="Logo" className="max-h-16 max-w-28 object-contain" onError={() => setPreview('')} />
          ) : (
            <span className="text-white font-black text-xl">UAT</span>
          )}
        </div>
        {/* Input */}
        <div className="flex-1 space-y-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Sube tu imagen a <a href="https://imgur.com" target="_blank" rel="noreferrer" className="text-uat-orange underline">imgur.com</a> y pega aquí la URL directa de la imagen
            </p>
          </div>
          <div className="flex gap-2">
            <button onClick={guardar} className="bg-uat-orange text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-uat-orange-light transition-colors">
              {guardado ? '✅ Guardado' : 'Aplicar logo'}
            </button>
            {preview && (
              <button onClick={quitar} className="border border-gray-300 text-gray-600 font-semibold px-4 py-2 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                Quitar logo
              </button>
            )}
          </div>
